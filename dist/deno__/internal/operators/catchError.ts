 ...
 * import { map, catchError } from 'rxjs/operators DENOIFY: DEPENDENCY UNMET (BUILTIN)';
 *
 * of(1, 2, 3, 4, 5).pipe(
 *     map(n => {
 *   	   if (n === 4) {
 ...
 * ```ts
 * import { of } from 'rxjs DENOIFY: DEPENDENCY UNMET (BUILTIN)';
 * import { map, catchError, take } from 'rxjs/operators DENOIFY: DEPENDENCY UNMET (BUILTIN)';
 *
 * of(1, 2, 3, 4, 5).pipe(
 *     map(n => {
 *   	   if (n === 4) {
